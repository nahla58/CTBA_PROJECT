...
def format_date_for_display(date_str: str) -> Dict[str, str]:
    ...
    try:
        ...
        # Convertir en Europe/Paris (UTC+1/UTC+2 selon DST)
        ...
        return {
            ...
            'timezone': f'Europe/Paris ({local_date.strftime("%Z")})',
            'iso_local': iso_local,
            'iso_utc': iso_utc
        }
...
def get_current_local_time() -> Dict[str, str]:
    ...
    try:
        ...
        return {
            ...
            'timezone': f'Europe/Paris ({local_now.strftime("%Z")})',
            'timestamp': iso_utc,
...
def import_from_msrc():
    ...
    try:
        ...
        # Get current date for bulletins
        now = datetime.utcnow()
        year = now.year
        month = now.month
        
        # Try multiple MSRC API endpoints
        # Primary: Current month bulletins
        cvrf_url = f"https://api.msrc.microsoft.com/cvrf/v2.0/updates/{year}-{month:02d}"
        
        logger.info(f"📡 Fetching MSRC bulletins from {cvrf_url}...")
        
        response = requests.get(cvrf_url, timeout=30)
        
        if response.status_code == 404:
            logger.info("⚠️ No bulletins for this month, trying previous month...")
            # Try previous month if current month has no bulletins
            if month == 1:
                prev_year = year - 1
                prev_month = 12
            else:
            ...
            cvrf_url = f"https://api.msrc.microsoft.com/cvrf/v2.0/updates/{prev_year}-{prev_month:02d}"
            response = requests.get(cvrf_url, timeout=30)
        ...
        # Extract CVEs from CVRF document
        documents = data.get('value', [])
        logger.info(f"📊 Found {len(documents)} MSRC bulletins")
        
        if not documents:
            logger.info("ℹ️ No MSRC bulletins available")
            return {'imported': 0, 'source': 'msrc', 'note': 'No bulletins available'}
        
        conn = sqlite3.connect('ctba_platform.db')
        cursor = conn.cursor()
        
        for doc in documents:
            # Each document is a bulletin with CVEs
            cve_ids = doc.get('cveID', [])
            if not cve_ids:
                continue
            
            # Get bulletin details
            title = doc.get('title', 'Microsoft Security Bulletin')
            doc_id = doc.get('ID', '')
            severity = doc.get('severity', 'UNKNOWN')
            affected = doc.get('affectedComponent', [])
            
            # Parse severity to CVSS
            cvss_score = 7.0  # Default for Microsoft bulletins
            if severity:
            ...
            # Process each CVE in the bulletin
            for cve_id in cve_ids:
                ...
                # Build description from bulletin info
                description = f"{title}. Bulletin: {doc_id}. "
                if affected:
                    components = [str(a.get('name', '')) for a in affected if a.get('name')]
                    if components:
                        description += f"Affected: {', '.join(components[:3])}"
                
                # Extract products using improved NLP
                products = nlp_extractor.extract_products(description, cve_id)
                ...
                cursor.execute("""
                    INSERT INTO cves (
                        ...
                        affected_products, references, source, status, created_at, updated_at
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    cve_id,
                    description[:5000],  # Limit description length
                    now_iso,
                    cvss_score,
                    _get_severity_from_cvss(cvss_score),
                    json.dumps([{'vendor': p.get('vendor', 'Unknown'), 
                               'product': p.get('product', 'Unknown')} for p in products]),
                    json.dumps([{"url": f"https://msrc.microsoft.com/update-guide/vulnerability/{cve_id}"}]),
                    'msrc',
                    'PENDING',
                    now_iso,
                    now_iso
                ...
                if import_count % 10 == 0:
                    logger.info(f"   📊 Imported {import_count} CVEs...")
        
        conn.commit()
        conn.close()
        
        logger.info(f"✅ MSRC import completed: imported {import_count} CVEs, skipped {duplicate_count} duplicates")
        return {'imported': import_count, 'duplicates': duplicate_count, 'source': 'msrc'}
    
    except requests.exceptions.Timeout:
        logger.warning("⚠️ MSRC API timeout - will retry in 30 minutes")
        return {'imported': 0, 'source': 'msrc', 'error': 'API timeout'}
    except requests.exceptions.RequestException as e:
        logger.warning(f"⚠️ MSRC API error: {e}")
        return {'imported': 0, 'source': 'msrc', 'error': 'API connection failed'}
    except Exception as e:
        logger.error(f"❌ MSRC import failed: {e}")
